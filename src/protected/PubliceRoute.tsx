...
export const NoAuthRoute = ({ element }: NoAuthRouteProps) => {
  ...
  if (!user) return element; // not logged in → allow access
  ...
  switch (user.role) {
    ...
    case "vendor":
      if (user.status === "verified") return <Navigate to="/vendor/dashboard" />;
      if (user.status === "pending" || user.status === "reviewing") {
