...
import { GuideLocalGuideBookingSection } from "@/components/client/local-guide-bookings/Guide/GuideLocalGuideBookingSection";
import { useLocalGuideBookingsForGuide } from "@/hooks/local-guide-booking/useGuideLocalGuideBooking";
import type {
...
const STATUS_OPTIONS: {
  value: LocalGuideBookingStatus | "all";
  label: string;
}[] = [
...
const PAYMENT_OPTIONS: {
  value: LocalGuidePaymentFilter | "all";
  label: string;
}[] = [
...
export default function GuideLocalGuideBookingsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    LocalGuideBookingStatus | "all"
  >("all");
  const [paymentFilter, setPaymentFilter] = useState<
    LocalGuidePaymentFilter | "all"
  >("all");
  ...
  const pendingQuery = useLocalGuideBookingsForGuide({
    category: "pending",
    page: pendingPage,
    ...sharedFilters,
  });

  const completedQuery = useLocalGuideBookingsForGuide({
    category: "completed",
    page: completedPage,
    ...sharedFilters,
  });

  const pendingData = pendingQuery.data as
    | LocalGuideBookingListResponse
    | undefined;
  const completedData = completedQuery.data as
    | LocalGuideBookingListResponse
    | undefined;
      ...
      <header className="space-y-4 rounded-3xl border border-slate-200 bg-white/80 p-5 shadow-sm">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Track all your bookings, monitor payment status, and manage your
            services.
          </p>
        </div>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white/80 p-5 shadow-sm">
        <div className="grid gap-4 lg:grid-cols-4">
          ...
          <Select
            value={statusFilter}
            onValueChange={(value) =>
              setStatusFilter(value as LocalGuideBookingStatus | "all")
            }
          >
          ...
          <Select
            value={paymentFilter}
            onValueChange={(value) =>
              setPaymentFilter(value as LocalGuidePaymentFilter | "all")
            }
          >
            <SelectTrigger className="rounded-2xl border-slate-200 bg-white text-sm">
        ...
        >
          ...
          <TabsContent value="completed" className="mt-0">
            <GuideLocalGuideBookingSection
              ...
              loadingMore={
                completedQuery.isFetching && completedQuery.isFetched
              }
