...
import { useCreateInstructions } from "@/hooks/guideInstructions/useInstructions";
...
const statusConfig = {
  ...
  cancellation_requested: { label: "Cancellation Requested", color: "bg-red-500", icon: XCircle },
...
export function BookingListGuide() {
  ...
  const {mutate : addInstruction,isPending} = useCreateInstructions()
  ...
  const handleInstructionSubmit = (instructionData : CreateInstructionDto)=>{
      addInstruction(instructionData,{
        onSuccess : (response) =>{
            toast.success(response.message);
        },
        onError : (error : any) =>{
          toast.error(error.response.data.message || "Failed to add instruction")
        }
      })
  }
