...
export const exportAdminSalesReportToPDF = ({ report, period, startDate, endDate }: ExportOptions) => {
  ...
  const warningColor: [number, number, number] = [251, 146, 60]; // Orange-400
  ...
  if (report.vendorBreakdown.length > 0) {
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...darkGray);
    doc.text("Revenue Breakdown by Vendor", 14, yPosition);
    yPosition += 12;
  ...
  if (report.revenueTrend.length > 0) {
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...darkGray);
    doc.text("Revenue Trend Over Time", 14, yPosition);
    yPosition += 12;
  ...
  if (report.profitVsCommission.length > 0) {
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...darkGray);
    doc.text("Profit vs Commission Analysis", 14, yPosition);
    yPosition += 12;
